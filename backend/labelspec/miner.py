...
不能直接修改 Rule，只生成供人工审核的建议。典型 Case 最多 8 条。
target_rule_id 优先指向现有 Rule；只有确实缺少规则时才为 null。
输出严格符合 JSON Schema。"""
...
class SpecGapMiner:
    ...
    async def mine(self, run_id: str) -> List[Dict[str, Any]]:
        ...
        failures = [
            item for item in self.store.list_annotations(run_id)
            if item["route"] in {"REVIEW", "AMBIGUOUS", "SPEC_GAP"}
        ]
        groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        for item in failures:
            boundary_ids = sorted(
                rule["rule_id"] for rule in item["disclosure"].get("boundaries", [])
            )
            signature = "|".join(sorted(item["candidates"])) + "::" + "|".join(boundary_ids)
