...
function GapsPage({ runs, refresh, notify }: { runs: Run[]; refresh: () => Promise<void>; notify: (text: string, error?: boolean) => void }) {
  ...
  async function savePatch() {
    ...
    try {
      ...
      if (!Array.isArray(operations) || operations.length === 0) throw new Error('Patch operations 必须是非空数组')
      await api.updateRulePatch(patchEditing.patch.id, undefined, {
        ...patchEditing.patch.payload,
        operations,
        reason: patchReason.trim() || patchEditing.patch.payload.reason || '人工修改 Rule Patch',
      })
...
function SettingsPage({ health, refresh, notify }: { health: { api_key_configured: boolean } | null; refresh: () => Promise<void>; notify: (text: string, error?: boolean) => void }) {
  ...
  return <>
    ...
    </div><div className="two-col"><div className="field"><label>AUTO_ACCEPT 置信度阈值</label><input className="input" type="number" min="0" max="1" step="0.01" value={settings.auto_accept_threshold} onChange={event => update('auto_accept_threshold', Number(event.target.value))} /></div><div className="field"><label>重复规则缺口聚类数</label><input className="input" type="number" min="2" step="1" value={settings.spec_gap_min_cluster_size} onChange={event => update('spec_gap_min_cluster_size', Number(event.target.value))} /><div className="hint">达到该数量时合并同类案例；Verifier 单案例缺口也会生成待审核 Patch。</div></div></div></div></div>}
  </>
}
